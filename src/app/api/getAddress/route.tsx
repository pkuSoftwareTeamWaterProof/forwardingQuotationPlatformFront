...
export async function POST(request: Request) {
    ...
    const prev_list = level > 0? JSON.parse(await fs.readFile(process.cwd() + '/src/addresses/children/json/' + level_list[level - 1] + '.json', 'utf8')): null
    const cur_list = JSON.parse(await fs.readFile(process.cwd() + '/src/addresses/children/json/' + level_list[level] + '.json', 'utf8'))
